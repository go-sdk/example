...
func init() {
	app.RegisterBootstrap(prepareStorage)
	app.RegisterRoute(http.MethodPost, "/api/v1/files", uploadFile)
	app.RegisterRoute(http.MethodGet, "/api/v1/files/{id}", downloadFile)
	app.RegisterRoute(http.MethodPut, "/api/v1/users/{id}/avatar", replaceAvatar)
}

func prepareStorage(context.Context, *dbx.DB) error {
	if err := os.MkdirAll(appconfig.StorageRoot(), 0o750); err != nil {
		return errx.Wrap(err, "create storage directory")
	}
	return nil
}

func uploadFile(c *standard.Context) error {
	if err := appauth.Require(c, "files.write"); err != nil {
		return err
	}
	value, path, err := receiveFile(c)
	if err != nil {
		return err
	}
...
func replaceAvatar(c *standard.Context) error {
	...
	cleanupReplacedAvatar(c, oldFile)
	return c.JSON(http.StatusOK, map[string]any{"data": fileResponseOf(value)})
}

func downloadFile(c *standard.Context) error {
	...
	data, err := os.ReadFile(filepath.Join(appconfig.StorageRoot(), value.StoredName))
...
func receiveFile(c *standard.Context) (model.File, string, error) {
	var value model.File
	filename, data, err := c.ReadFormFile("file", appconfig.MaxUploadBytes())
	if err != nil {
		return value, "", err
	}
	...
	path := filepath.Join(appconfig.StorageRoot(), storedName)
...
func cleanupReplacedAvatar(c *standard.Context, oldFile model.File) {
	if oldFile.Id == "" {
		return
	}
	...
	if err := os.Remove(filepath.Join(appconfig.StorageRoot(), oldFile.StoredName)); err != nil && !errors.Is(err, os.ErrNotExist) {
...
var _ standard.HandlerFunc = uploadFile
var _ standard.HandlerFunc = downloadFile
var _ standard.HandlerFunc = replaceAvatar
