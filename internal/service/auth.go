...
func NewAuth(config appconfig.Config) *Auth {
	return &Auth{
		config: config,
	}
}
...
func (s *Auth) Login(ctx context.Context, req *appv1.LoginReq) (*appv1.LoginResp, error) {
	...
	token, expiresAt, err := appauth.Sign(user, []byte(s.config.Auth.JWTSecret), s.config.Auth.ExpiresIn)
	if err != nil {
		return nil, err
	}
	return &appv1.LoginResp{
		AccessToken: token,
...
func invalidCredentials() error {
	return standard.ErrUnauthenticated.WithErrorCode(commonv1.ErrorCode_ERROR_CODE_INVALID_CREDENTIALS)
}
